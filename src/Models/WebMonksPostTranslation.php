...
use WebMonksBlog\Laravel\Fulltext\Indexable;
use Illuminate\Database\Eloquent\Model;
use WebMonksBlog\Interfaces\SearchResultInterface;

class WebMonksPostTranslation extends Model implements SearchResultInterface
{
    use Sluggable;
    use Indexable;
    ...
    public function image_url($size = 'medium')
    {
        ...
        return asset(config("webmonksblog.blog_upload_dir", "blog_images") . "/" . $filename);
    }
    ...
    {
        if (config("webmonksblog.use_custom_view_files") && $this->use_view_file) {
            // using custom view files is enabled, and this post has a use_view_file set, so render it:
            $return = view("webmonksblog::partials.use_view_file", ['post' => $this])->render();
        } else {
        ...
        if (!config("webmonksblog.echo_html")) {
            // if this is not true, then we should escape the output
            if (config("webmonksblog.strip_html")) {
                $return = strip_tags($return);
            }

            $return = e($return);
            if (config("webmonksblog.auto_nl2br")) {
                $return = nl2br($return);
            }
        }
    ...
    protected function check_valid_image_size(string $size = 'medium')
    {
        ...
        if (array_key_exists("image_" . $size, config("webmonksblog.image_sizes"))) {
            return true;
        }
        ...
        if (starts_with($size, "image_")) {
            ...
            /* the config/webmonksblog.php and the DB columns SHOULD have keys that start with image_$size
            however when using methods such as image_url() or has_image() it SHOULD NOT start with 'image_'

            To put another way: :
                in the config/webmonksblog.php : config("webmonksblog.image_sizes.image_medium")
                in the database table:    : webmonksblog_posts.image_medium
                when calling image_url()  : image_url("medium")
            */
            throw new \InvalidArgumentException("Invalid image size ($size). WebMonksPost image size should not begin with 'image_'. Remove this from the start of $size. It *should* be in the webmonksblog.image_sizes config though!");
        }

        throw new \InvalidArgumentException("WebMonksPost image size should be 'large','medium','thumbnail' or another field as defined in config/webmonksblog.php. Provided size ($size) is not valid");
    }
    ...
    {
        return $routeWithoutLocale ? route("webmonksblog.single", ["", $this->slug]) : route("webmonksblog.single", [$loacle, $this->slug]);
    }
    ...
    {
        return route("webmonksblog.admin.edit_post", $this->post_id);
    }
}
