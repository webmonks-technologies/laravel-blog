...
class Move {
  ...
  * Destination node
  *
  * @var \WebMonksBlog\Baum\Node | int
  */
  ...
  */
   ...
   *
   * @param   \WebMonksBlog\Baum\Node      $node
   * @param   \WebMonksBlog\Baum\Node|int  $target
   ...
   * supplied parameter is an instance of \WebMonksBlog\Baum\Node. Otherwise it will try
   * to find the node in the database.
   *
   * @param   \WebMonksBlog\Baum\node|int
   * @return  \WebMonksBlog\Baum\Node
   */
  protected function resolveNode($node) {
    if ( $node instanceof \WebMonksBlog\Baum\Node ) return $node->reload();
