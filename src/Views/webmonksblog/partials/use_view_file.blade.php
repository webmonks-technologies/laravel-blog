...
@else
    ...
    @if(\Auth::check() && \Auth::user()->canManageWebMonksBlogPosts())
         {{--is logged in + canManageWebMonksBlogPosts() == true, so show a detailed error--}}
        <div class='alert alert-danger'>Custom blog post blade view file
                        (<code>{{$post->full_view_file_path()}}</code>) not found. <a
                    href='https://github.com/webmonks/laravel-blog'
                    target='_blank'>See Laravel Blog Package help here</a>.
                    </div>

    @else
        {{--is not logged in, or User::canManageWebMonksBlogPosts() for current user == false--}}
