...
abstract class BaseWebMonksBlogPostRequest extends BaseRequest
{
    ...
    protected function baseBlogPostRules()
    {
        ...
        if (config('webmonksblog.use_custom_view_files')) {
        ...
        foreach ((array)config('webmonksblog.image_sizes') as $size => $image_detail) {
            if ($image_detail['enabled'] && config("webmonksblog.image_upload_enabled")) {
