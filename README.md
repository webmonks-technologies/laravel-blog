...
`composer require webmonks/laravel-blog`
...
`php artisan vendor:publish --provider="WebMonksBlog\WebMonksBlogServiceProvider"`
...
```
 /**
     ...
     */
    public function canManageWebMonksBlogPosts()
    {
...
To see package on Packagist click this [Link](https://packagist.org/packages/webmonks/laravel-blog)

### Single Language Version
To install the single language version of the package use version v8.1x:

1- `composer require webmonks/laravel-blog:v8.2.0`

2- `php artisan vendor:publish --provider="WebMonksBlog\WebMonksBlogServiceProvider"`
...
## Important Notes
- For laravel 8.x's default auth User model, change user model in `webmonksblog.php` to: `\App\Models\User::class`
...
- fully configurable via its `config/webmonksblog.php` config file
...
After doing the correct `vendor:publish`, all of the default template files will be found in /resources/views/vendor/webmonksblog/ and are easy to edit to match your needs.

### Customizing admin views
If you need to customize the admin view, just copy the files from
`vendor/webmonksblog/src/Views/webmonksblog_admin`
to
`resources/views/vendor/webmonksblog_admin`
Then you can modify them just like any other view file.

## Routes

It will auto set all required routes (both public facing, and admin backend). There are some config options (such as changing the /blog/ url to something else), which can be done in the webmonksblog.php file.

## Config options
All config options have comments which describe what they do. Please just refer to the `webmonksblog.php` file in your /config/ dir.
...
See the config/webmonks.php captcha section. There is a built in system (basic!) that will prevent most automated spam attempts.
...
If you want to write your own implementation then create your own class that implements \WebMonksBlog\Interfaces\CaptchaInterface, then update the config/webmonksblog.php file (change the captcha_type option).
...
What view file should the webmonks::partials.add_comment_form view include? You can set this to whatever you need, and then create your own view file. The default included basic captcha class will return "webmonks::captcha.basic".
...
This isn't part of the interface, it isn't required. By default it does nothing. But you can put some code in this method and it'll be run in the WebMonksReaderController::viewSinglePost method.
